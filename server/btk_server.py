...
TARGET_ADDRESS = "64:41:E6:7C:41:0A"

class BTKbDevice():
    # change these constants
    MY_ADDRESS = "DC:A6:32:81:24:1D"
    MY_DEV_NAME = "Gamehawk"
...
class BTKbService(dbus.service.Object):
    ...
    def send_mouse(self, modifier_byte, keys):
        # 0xA1 = Input Report, 2 = Mouse Report ID
        state = [0xA1, 2] 
        # Append the payload received from the client
        for key_code in keys:
            state.append(int(key_code))
            
        # Ensure we don't send too much garbage if something is wrong, 
        # but for Abs Mouse we expect 6 bytes of payload (Buttons, X_lo, X_hi, Y_lo, Y_hi, Wheel)
        # resulting in a total state length of 8.
        self.device.send_string(state)
